...
def construct_student_bp(controller):
    ...
    def student():
        studentList = controller.raw("SELECT users.id, users.status, users.firstName, users.lastName, users.email, majors.majorName FROM users LEFT JOIN majors ON majorId = majors.id WHERE users.status = 3 ORDER BY users.id").fetchall()
